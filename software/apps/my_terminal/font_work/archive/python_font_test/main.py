...
LABEL_FONT_SIZE = 8
LABEL_FONT = ImageFont.load_default()  # Simple 6x10 bitmap font
LABEL_COLOR = (255, 255, 0)            # Yellow label for visibility
...
def build_preview(font_data, face):
    ...
    for i, _ in enumerate(font_data):
        ...
        # Draw index label
        label = f"{i:3}"  # Pads numbers like ' 47'
        draw.text((x + 1, y + 1), label, font=LABEL_FONT, fill=LABEL_COLOR)
        
        if i == 247:
            draw.rectangle([x, y, x + CHAR_WIDTH - 1, y + CHAR_HEIGHT - 1], outline="red")

    img.save(PREVIEW_FILE)
    print(f"✅ Saved labeled preview to {PREVIEW_FILE}")
